...
--
SET TERM OFF
SET ECHO OFF FEED OFF VER OFF HEA OFF PAGES 0 COLSEP ', ' LIN 32767 TRIMS ON TRIM ON TI OFF TIMI OFF ARRAY 100 NUM 20 SQLBL ON BLO . RECSEP OFF;
...
-- If previous query fails, do not ask for ecr_min_snap_id
SELECT '' ecr_min_snap_id FROM dual WHERE ROWNUM = 0;
...
SELECT /*+ INLINE &&ecr_sq_fact_hints_9i. */
       ...
       ROUND(PERCENTILE_DISC(0.50) WITHIN GROUP (ORDER BY (r_bytes + w_bytes) / POWER(2, 20) / elapsed_sec)) rw_mbps_median,
...
HOS zip -mT esp_requirements_&&esp_host_name_short..zip esp_requirements_stp_&&esp_host_name_short._&&ecr_collection_key..csv >/dev/null

UNDEF ecr_sq_fact_hints_9i ecr_date_format useappend skip_on_9i
UNDEF esp_host_name_short ecr_collection_key ecr_dbid ecr_instance_number ecr_min_snap_id ecr_collection_host